function aboxes = do_faster_rcnn_test_eval(conf, do_nms,model, imdb, roidb, cache_name, method_name)
    if  strcmp(method_name,'FRCN-stage1')
        load('./output/fast_rcnn_cachedir/faster_rcnn_caltech_vgg_16layers_top-1_nms0_7_top2000_stage1_fast_rcnn/test/pedestrian_boxes_test.mat');
    end
    
    if strcmp(method_name,'FRCN-stage2')
        load('./output/fast_rcnn_cachedir/faster_rcnn_caltech_vgg_16layers_top-1_nms0_7_top2000_stage2_fast_rcnn/test/pedestrian_boxes_test.mat');
    end
    %loading the result
    aboxes=boxes;
    ...
    cache_dir = fullfile(pwd, 'output', conf.exp_name, cache_name);
    res_boxes = aboxes;
    ...
    folder1 = fullfile(pwd, 'output', conf.exp_name, cache_name, method_name);
