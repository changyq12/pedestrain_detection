function model = adaBoostTrain_trans( X0, X0_score, X1, X1_score, varargin )
...
lo_bound = 0.0001;
hi_bound = 1 - lo_bound;
X0_score = max(min(hi_bound, X0_score), lo_bound);
X1_score = max(min(hi_bound, X1_score), lo_bound);
% thres = 0.5;
% err = (sum(X0_score < thres)+sum(X1_score >= thres)) / (length(X0_score) + length(X1_score))
h0 = 0.5*log(X0_score./(1-X0_score));
h1 = 0.5*log(X1_score./(1-X1_score));
